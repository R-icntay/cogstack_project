...
This assesses the distinctiveness of keywords, by looking at keyword overlap. Specifically, it measures the fraction of keywords unique to a class, averaged across all classes
...
Coverage of keywords within a class relative to the coverage across the class boundary of unrelated documents. Not sure how this would work if all tags are in all documents.
...
```{r}
mm=map(seq_len(length(files)), ~ read_csv(files[.x], show_col_types = FALSE) %>% 
         mutate(sentence_mdl_id = .x)) %>% 
  bind_rows()
```
...
# Calculate average attributions token scores per reshuffle 
  group_by(cancer_tokens = tokens, res_id) %>% 
  summarise(
    n_occur = n(),
    cancer_mean_attr_score = mean(attr_scores)) %>% 
  ungroup() %>% group_by(res_id) %>% 
# Take 100 top words per doc
  slice_max(cancer_mean_attr_score, n = 100) %>% 
  ungroup() %>%
# Find how many times a keyword occurs across across all
# resamples
  add_count(cancer_tokens, name = "n_across_resamples") %>% 
  ungroup() %>% 
...
# Keywords
agg_cancer_attr %>% 
  group_by(cancer_tokens) %>% 
  filter(select_threshold > 0.5) %>% 
  #distinct(cancer_tokens) %>% 
  View()
```

```{r}
kk=list(distinct(kw_cancer, tokens, .keep_all = TRUE), distinct(kw_cell, tokens, .keep_all = TRUE), distinct(kw_organ, tokens, .keep_all = TRUE), distinct(kw_gene, tokens, .keep_all = TRUE)) %>% 
  bind_rows() %>% 
  select(tokens, tag) %>% 
  add_count(tag, name = "kw_in_class")
```
...
distinct_measure <- function(kw_list){
  ...
  calc_distinct_keywords = function(tag, all_kws){
    discard(kk %>% filter(tag == {{tag}}) %>% pull(tokens), (kk %>% filter(tag == {{tag}}) %>% pull(tokens)) %in% (kk %>% filter(tag != {{tag}}) %>% pull(tokens))) %>% length(.) %>%  tibble(tag = tag, distinct_kws = .)
}
  ...
  purrr::map_dfr(all_tags, ~ calc_distinct_keywords(.x, all_kws = all_kws)) %>% 
    left_join(all_kws %>% distinct(tag, kws_in_class)) %>% 
...
View(
  kk %>% 
    group_by(tag) %>% 
    filter((tokens %in% keep()))
)
```

```{r}
map_dfr(all_tags, ~ calc_dskw(.x, kk = kk)) %>% 
  left_join(kk %>% distinct(tag, kw_in_class)) %>% 
  mutate(class_dm = distinct_kws / kw_in_class) %>% 
  mutate(overall_distinctive_measure = mean(class_dm)) %>% 
  View()
```
...
```{r}
keep(kk %>% filter(tag == "Gene_or_gene_product") %>% pull(tokens), (kk %>% filter(tag == "Gene_or_gene_product") %>% pull(tokens)) %in% (kk %>% filter(tag != "Gene_or_gene_product") %>% pull(tokens))) 
```
